...
class Aggregator(object):
    ...
    def __call__(self, self_vectors, neighbor_vectors, neighbor_relations, AppH_embeddings):
        outputs = self._call(self_vectors, neighbor_vectors, neighbor_relations, AppH_embeddings)
        return outputs

    @abstractmethod
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, AppH_embeddings):
        # dimension:
    ...
    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, AppH_embeddings):
        ...
        if not avg:
            # [batch_size, 1, 1, dim]
            AppH_embeddings = tf.reshape(AppH_embeddings, [self.batch_size, 1, 1, self.dim])

            # [batch_size, -1, n_neighbor]
            AppH_relation_scores = tf.reduce_mean(AppH_embeddings * neighbor_relations, axis=-1)
            AppH_relation_scores_normalized = tf.nn.softmax(AppH_relation_scores, axis=-1)

            # [batch_size, -1, n_neighbor, 1]
            AppH_relation_scores_normalized = tf.expand_dims(AppH_relation_scores_normalized, axis=-1)

            # [batch_size, -1, dim]
            neighbors_aggregated = tf.reduce_mean(AppH_relation_scores_normalized * neighbor_vectors, axis=2)
        else:
            # [batch_size, -1, dim]
    ...
    def __init__(self, batch_size, dim, dropout=0., act=tf.nn.relu, name=None):
        ...
        with tf.variable_scope(self.name):
            self.weights = tf.get_variable(
                shape=[self.dim + self.dim, self.dim], initializer=tf.contrib.layers.xavier_initializer(), name='weights')
            self.bias = tf.get_variable(shape=[self.dim], initializer=tf.zeros_initializer(), name='bias')

    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, AppH_embeddings):
        # [batch_size, -1, dim]
        neighbors_agg = self._mix_neighbor_vectors(neighbor_vectors, neighbor_relations, AppH_embeddings)

        # [-1, dim]
        # output = tf.reshape(self_vectors + neighbors_agg, [-1, self.dim])
        output = tf.concat([self_vectors, neighbors_agg], axis=2)
        output = tf.reshape(output, [-1,self.dim+self.dim])
        print(output)
